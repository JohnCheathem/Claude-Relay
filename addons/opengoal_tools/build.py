...
def write_startup_gc(commands):
    ...
    udir = _user_dir()
    # user.gc is compiled with allow_emit=false (no code generation allowed).
    # define-extern generates code and triggers the REPL warning
    # "Compilation generated code, but wasn't supposed to".
    # bg, bg-custom, *artist-all-visible* are already in the game's symbol
    # table when connected via (lt), so no forward declarations are needed.
